...
HOOK_PATTERNS = [
    ...
    # 더 구체적인 신호(반전어, 극단 수식어)를 앞에 두고, 넓은 규칙을 뒤에 둡니다.
    ("반전형", r"알고\s*보니|사실은|그런데\s*반전|반전|결과는|근데\s*이게|의외로|줄\s*알았"),
    ("극단수식형", r"역대급|레전드|세계\s*최초|세계\s*최|사상\s*최|최강|최악|극한|미친|１위|\b1위\b|가장\s*[가-힣]+한"),
...
def load_api_key(explicit_file: str | None) -> str:
    ...
    shared = shared_keys().get("YOUTUBE_API_KEY")
    if shared:
        return shared
