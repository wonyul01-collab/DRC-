...
| Hermes (사무실 PC) | **데스크톱 앱** (로컬) | 수동 작업 · 마케팅 단장브리핑 잡 |
...
- **데스크톱 앱 Hermes 는 웹 UI 에 안 보인다.** VPS 웹 화면의 세션·채널
  목록에는 그 서버의 것만 나온다. 사무실 PC 앱의 세션·크론·텔레그램 연결은
  그 PC 에서만 확인할 수 있다. 진단 대상을 헷갈리면 시간만 버린다.
- **데스크톱 앱에 건 크론은 PC 가 켜져 있을 때만 돈다.** 앱을 닫거나 PC 를
  끄면 그대로 멈춘다. 정기 발송물은 VPS 에 걸어야 한다.
